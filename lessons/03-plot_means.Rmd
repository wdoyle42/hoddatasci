...
To do that, we need to reorder the states in the order of debt levels. To do that, we need to work with the state variabl `stabbr` as a factor.
...
We can also use facets when there are multiple groups, so for instance if we wantto work across regions:

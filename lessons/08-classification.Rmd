...
za%>%
  ...
  mutate(got_pizza_f=as_factor(ifelse(requester_received_pizza==TRUE,"Yes","No")))->za
...
logit_class%>%
  predict(za_test)%>%
  bind_cols(za_test)%>%
  spec(truth=got_pizza_f,estimate=.pred_class)
...
logit_class%>%
  ...
  bind_cols(za_test)%>%
  threshold_perf(truth=got_pizza_f,estimate=.pred_Yes,thresholds=seq(0,1,by=.1))%>%View()

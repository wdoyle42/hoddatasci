...
player_team<-page%>%
  html_nodes(my_nodes)%>%
  html_text()
#  stringr::str_extract("[\\w]+[\\s]+[\\w]+[^/t]") #You can use this too: gets text
...
# TOS ask for a 3s crawl delay: NP! https://www.sports-reference.com/robots.txt
...
load("../working/zipCityCountyStateMap.Rda")
...
names(dav_df)<-c("zip","college.educ","income75")
...
```{r}
my_zillow_key<-readLines("zillow_key.txt")
zwsid<-my_zillow_key
...
zdemodata=tibble(list(zip=character(),medListPrice=numeric(),medValSqFt=numeric()))
...
mod1<-lm(medValSqFt~college.educ+income75,data=dav_house_df);summary(mod1)
...
davidson_zipshp$zip<-davidson_zipshp$id

davidson_map<-left_join(dav_house_df,davidson_zipshp)

address="Davidson County, TN"

#Pull Map from Google Maps
dav_gmap=get_googlemap(center=address,maptype=c("roadmap"),zoom=10,color="bw")
```

## Income by Zip Code in Davidson County, TN
```{r}

## These aren't working, need to fix. 
#Plot Income by Zip Code
g3<-ggmap(dav_gmap)
g3<-g3+geom_polygon(data=filter(davidson_map,medValSqFt>55),
                    aes(x=long,y=lat,group=zip,
                                       fill=income75),

...
First, let's git rid of unwanted columns by selecting a list of things we don't want, using the `slice(-c(element1,element2,...))` or `select(slice(-c(element1,element2,...)))` setup. 
...
if(file.exists("free.xls")==FALSE){
  ...
  download.file("http://nces.ed.gov/programs/digest/d14/tables/xls/tabn302.10.xls",destfile="coll_rates.xls")

  coll<-read_excel("coll_rates.xls",skip=21,col_names=FALSE)  
...
#Get rid of unwanted rows
coll2<-coll%>%filter(is.na(coll[,1])==FALSE)


##Only years we eant
coll2<-coll2%>%slice(1:41)

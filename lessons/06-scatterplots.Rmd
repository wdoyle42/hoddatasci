...
cex_sum<-cex%>%mutate(inc_rank_r=round(inc_rank))%>%group_by(inc_rank_r)%>%summarize(store_mean=mean(other_store,na.rm=TRUE))

g1a<-ggplot(cex_sum,aes(x=inc_rank_r,y=store_mean))

g1a<-g1a+geom_point()

g1a<-g1a+ylab("Store  Spending")+xlab("Income Rank")+

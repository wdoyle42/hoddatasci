...
*Quick Exercise*
Choose a different college and two different things about that college. Have R print the output. 
...
sc%>%
  group_by(control)%>%
  summarize(mean_adm_rate=mean(adm_rate))
```
...
```{r}
gg<-ggplot(data=sc,aes(x=sat_avg,y=costt4_a))
gg<-gg+geom_point()
gg<-gg+geom_smooth()
gg<-gg+facet_wrap(~control)
gg

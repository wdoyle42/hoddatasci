...
sc%>%filter(instnm=="Harvard University")
...
sc%>%filter(instnm=="New York University")%>%select(debt_mdn,md_earn_wne_p6)
...
```{r}
sc%>%filter(sat_avg>1200)%>%summarize(mean_earnings=mean(md_earn_wne_p6,na.rm=TRUE))
```

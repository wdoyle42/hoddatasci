...
Understnding which families will spend money on groceries is a key analytic problem for our organization. In particular,
...
cex_sum<-cex_sum%>%mutate(fam_income_level=as.factor(fam_income_level))

gg<-ggplot(cex_sum,aes(x=fct_reorder(f=fam_income_level,groc_fam),y=groc_fam,fill=fam_income_level))
...
gg<-ggplot(cex_sum,aes(x=fct_reorder(f=childage,groc_fam),y=groc_fam,fill=fct_reorder(f=childage,groc_fam)))
